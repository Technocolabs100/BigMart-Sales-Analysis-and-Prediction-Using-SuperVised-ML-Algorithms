...
def null_columns(train, test):
    """
    Print the percentage of null values in each column of the provided train and test datasets.
    ...
    Prints:
    - Percentage of null values for each column in the training dataset.
    - Percentage of null values for each column in the testing dataset.
    """
    # Display null values for the training dataset
    print("Train Nulls: ", [f"{(i, v / train.shape[0])}\n" for i, v in train.isna().sum().items() if v != 0])

    # Display null values for the testing dataset
    print("Test Nulls : ", [f"{(i, v / test.shape[0])}\n" for i, v in test.isna().sum().items() if v != 0])
